...
hex_to_BCD ; you can allocate on stack an array of 5 bytes and pass the pointer to start (below SP) or use DEFS
    ;note we are using UNPACKED bcd which *** NEEDS 10 BYTES ***
    ;args: r0 - hex number, r1 - pointer to BCD string to fill; 32 bit number need 10 BCD digits - 10 bytes. (unpacked)
...
strcpy ;fine, if you REALLY dont know, this copies one null-terminated string into another memory location
    ;r0: destination pointer
    ;r1: source string - must be null terminated
    ;this will copy the null char for you aswell
    push {r0-r2}
    strcpy_loop
        ldrb r2, [r1]
        cmp r2, #0
...
;-------- Problem with the Board/ST1/FPGA
;
; so it appears that after I upload an FPGA design, the bit for ST1 is set permanently high, and I cant use it anymore for this exercise.
; this code works, but only before I upload my FPGA design.
; Though I dont understand how the FPGA can affect the ST1 button.
;--------
...
st3_poll_hdl
    ;this function will handle top right btn/ST1 polling and place ST1 keypress events into the event fifo (fifo.s)
    ;r0 - boolean for current status of button. 0-> not pressed, 1-> pressed
    push {r0-r2, lr}
    adr r2, st3_status
    ldrb r1, [r2]

    ;a key press is detected when st1_status = 0 and r0 = 1, i.e. st1_status - r0 < 0 (signed)
    cmp r1, r0
    mov r1, r0 ;preserve current st1 status into r1 after bl to fifo_push (changing r0)
    bge st3_poll_hdl_end
    mov r0, #3
    bl fifo_push
...
keypad_poll_handler
    ...
    ;iii X oooo xxxxxxxx ([i]nput [o]utput)
    mov r2, #0 ; <= i < 3 
...
;-------- time-hex conversions --------;

;Time string format: "hh:mm:ss" - 9 bytes

;---- hex values for time units ----;

time_24hr_ms DEFW 0x5265c00 ;24hr in ms

time_24hr DEFW 86400 ;24 hours in hex / time values can be upto this value
time_1hr DEFW 3600
time_1min DEFW 60
time_1sec DEFW 1

hex_to_time
    ;R0 - hex value for time in SECONDS
    ;R3 - address of a 9 byte string
    push {r0-r3, lr}
    ;(ik theres repeated code, but this is easier/faster)
    ldr r2, time_24hr
    cmp r0, r2 ;values equal or greater to this cant be represented with 24hr time format
    bhs sys_abort_data

    ;need to calculate the value of every digit
    ...
    ;10hr digit
        ...
        add r0, r0, #48 ;get equivalent ascii code
        strb r0, [r3]
        add r3, r3, #1 ;next digit

    ;1hr  ...
        mov r0, r2 ; mov remainder from previous divide into arg so as to be divided
        ldr r1, time_1hr
        bl divide
        add  r0, r0, #48
        strb r0, [r3]
...
time_to_hex
    ;R0 - return register
    ;R1 - address to time-formatted string: hh:mm:ss
    ...
    ldrb r3, [r1]
    sub r3, r3, #48
    mla r0, r2, r3, r0 ;multiply-accumulate: r0 = (r2*r3) + r0
    ...
    add r1, r1, #2 ;skip over ':' seperator
